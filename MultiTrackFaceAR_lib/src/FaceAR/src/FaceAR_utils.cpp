...
void get_video_input_output_params(vector<string> &input_video_files, vector<string> &depth_dirs,
                                   vector<string> &output_pose_files, vector<string> &output_video_files,
                                   vector<string> &output_2d_landmark_files, vector<string> &output_3D_landmark_files,
                                   bool& camera_plane_pose, vector<string> &arguments)
{
...
void matchTemplate_m(  const Mat_<float>& input_img, Mat_<double>& img_dft, cv::Mat& _integral_img, cv::Mat& _integral_img_sq, const Mat_<float>&  templ, map<int, Mat_<double> >& templ_dfts, Mat_<float>& result, int method )
{
    ...
    FaceARTracker::crossCorr_m( input_img, img_dft, templ, templ_dfts, result);
...
//    edges.push_back(pair<int,int>(7,6));
    ...
    ////TODO:coordinate
    edges.push_back(pair<int,int>(8,9));
    edges.push_back(pair<int,int>(8,10));
    edges.push_back(pair<int,int>(8,11));

    Mat_<double> box = Mat(/*8*/12, 3, CV_64F, boxVerts).clone() * 100;

    Matx33d rot = FaceARTracker::Euler2RotationMatrix(Vec3d(pose[3], pose[4], pose[5]));
	Mat_<double> rotBox;
...
vector<cv::Point2d> CalculateLandmarks(FaceAR& facear_model)
{

    int idx = facear_model.patch_experts.GetViewIdx(facear_model.params_global, 0);
    return CalculateLandmarks(facear_model.detected_landmarks, facear_model.patch_experts.visibilities[0][idx]);
	...
	if(n >= 66)
	{
		...
		{		
			if(visibilities.at<int>(i))
			{
                ...
                cv::circle(img, featurePoint, 1, Scalar(255,0,0), 1.3*thickness_2);

                /// Keegan.Ren
                /// TODO: plot the face point and point number in the image
                char str_i[11];
                sprintf(str_i,"%d",i);
                cv::putText(img, str_i, featurePoint, CV_FONT_HERSHEY_COMPLEX, 0.3, cv::Scalar(0,255,0));
			}
		}
	}
...
void Draw(cv::Mat img, FaceAR& facear_model)
{
    ...
    int idx = facear_model.patch_experts.GetViewIdx(facear_model.params_global, 0);
    Draw(img, facear_model.detected_landmarks, facear_model.patch_experts.visibilities[0][idx]);
    for(int i = 0; i < facear_model.hierarchical_models.size(); ++i)
	{
        if(facear_model.hierarchical_models[i].pdm.NumberOfPoints() != facear_model.hierarchical_mapping[i].size())
		{
            Draw(img, facear_model.hierarchical_models[i]);
		}
	}
}
...
{
    Matx33d rotMatrix = FaceARTracker::Euler2RotationMatrix(euler);
	Vec3d axis_angle;
...
bool DetectSingleFace(Rect_<double>& o_region, const Mat_<uchar>& intensity_image, CascadeClassifier& classifier, cv::Point preference)
{
    ...
    bool detect_success = FaceARTracker::DetectFaces(face_detections, intensity_image, classifier);
...
bool DetectSingleFaceHOG(Rect_<double>& o_region, const Mat_<uchar>& intensity_img, dlib::frontal_face_detector& detector, double& confidence, cv::Point preference)
{
    ...
    bool detect_success = FaceARTracker::DetectFacesHOG(face_detections, intensity_img, detector, confidences);
